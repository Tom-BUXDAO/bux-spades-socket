...
export default function GameTable({ 
  ...
  const [handScores, setHandScores] = useState<{team1: any, team2: any} | null>(null);
  ...
  useEffect(() => {
    if (typeof document !== 'undefined') {
      ...
      // Auto-request fullscreen on mobile/tablet
      const isMobileOrTablet = screenSize.width < 1024;
      if (isMobileOrTablet) {
        // Try to enter fullscreen mode when component mounts on mobile/tablet
        const enterFullscreenBtn = document.createElement('button');
        enterFullscreenBtn.style.display = 'none';
        document.body.appendChild(enterFullscreenBtn);
        
        enterFullscreenBtn.addEventListener('click', handleFullscreenRequest);
        enterFullscreenBtn.click();
        
        // Clean up
        return () => {
          enterFullscreenBtn.removeEventListener('click', handleFullscreenRequest);
          document.body.removeChild(enterFullscreenBtn);
        };
      }
    }
  }, [screenSize.width]);

  // Calculate header height to prevent top from being cut off
  const headerHeight = Math.max(48, Math.floor(56 * scaleFactor));
  ...
  return (
    <>
      ...
      <div className="flex flex-col h-screen bg-gray-900 overflow-hidden">
        {/* Header */}
        <div 
          className="bg-gray-800 text-white px-4 py-1 flex justify-between items-center"
          style={{ 
            fontSize: `${Math.floor(16 * scaleFactor)}px`,
            height: `${headerHeight}px`,
            paddingTop: `${Math.max(4, Math.floor(8 * scaleFactor))}px`
          }}
        >
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          {/* Game table area - adjust width based on screen size */}
          <div className={`${screenSize.width < 768 ? 'w-full' : 'w-[70%]'} p-2 flex flex-col`}>
            {/* Game table */}
            ...
            <div className="bg-gray-800/50 rounded-lg relative" 
                style={{ 
                  height: `${Math.floor(Math.max(100, 120 * scaleFactor))}px`, 
          ...
          {/* Chat area - hide on mobile, show on tablet/desktop */}
        ...
        {/* Mobile only: Add a chat toggle button */}
        {screenSize.width < 768 && (
        ...
        {/* Mobile only: Chat overlay */}
        {screenSize.width < 768 && showMobileChat && (
